...
namespace DarkJimmy.Characters.Inputs
{
    public class Thumbstick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
	{
		...
		void Update()
		{
			...
			thumbImage.localPosition = Vector3.Lerp(thumbImage.localPosition, center, Time.deltaTime * smoothing);

			if (Vector3.Distance(center, thumbImage.localPosition) < .1f)
			{
				thumbImage.localPosition = center;
		...
		void MoveThumbImage(Vector2 position)
		{
			position.x += center.x;
			position.y += center.y;
			thumbImage.localPosition = position;
		}
